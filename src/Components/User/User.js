    ...
    return(
      <div className="user" onMouseOver={()=> document.getElementById('user-dropdown').style.display="grid"} onMouseOut={()=> document.getElementById('user-dropdown').style.display="none"}>
        ...
        <div id="user-dropdown"  >
          ...
          <h4 onClick={() => {localStorage.clear() ; window.open("http://localhost:3000/", "_self")}} style={{marginTop:"10px", display: "flex",color: "var(--red)",cursor: "pointer", flexDirection: "column", alignItems: "end"}}>LOGOUT</h4>
        </div>
      </div>
    )
  }
