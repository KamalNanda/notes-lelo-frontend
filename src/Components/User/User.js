    ...
    return(
      <div className="user" onMouseOver={()=> document.getElementById('user-dropdown').style.display="grid"} onMouseOut={()=> document.getElementById('user-dropdown').style.display="none"}>
        ...
        <div id="user-dropdown"  >
          ...
          <h4 onClick={() => {localStorage.clear() ; sessionStorage.clear(); window.open("https://notes-lelo-frontend.netlify.app", "_self")}} style={{marginTop:"10px", display: "flex",color: "var(--red)",cursor: "pointer", flexDirection: "column"}}>LOGOUT</h4>
        </div>
      </div>
    )
  }
