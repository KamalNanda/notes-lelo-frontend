...
const User = () => {
  ...
  return(
    <div className="user" onMouseOver={()=> document.getElementById('user-dropdown').style.display="grid"} onMouseOut={()=> document.getElementById('user-dropdown').style.display="none"}>
      <div className="user-header"><h4  onClick={() => history.push({pathname: `/courses/${user.course}/${user.sem.title}`, state:{ sem: user.sem, link : user.course }})}>Semester - {user.sem.title}</h4> <img src={user.imgUrl} id="userImg" alt="user-img" /></div>
